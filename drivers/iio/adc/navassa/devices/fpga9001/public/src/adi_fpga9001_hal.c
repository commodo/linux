...
int32_t adi_fpga9001_RegisterFieldRead(adi_fpga9001_Device_t *device,
    const uint32_t addr,
    uint32_t *fieldVal,
    uint32_t mask,
    uint8_t startBit)
{
    ...
    ADI_EXPECT(adi_fpga9001_RegisterRead, device, addr, &rdData);
...
int32_t adi_fpga9001_RegisterFieldWrite(adi_fpga9001_Device_t *device, const uint32_t addr, uint32_t fieldVal, uint32_t mask, uint8_t startBit)
{
    ...
    ADI_EXPECT(adi_fpga9001_RegisterRead, device, addr, &registerVal);

    /* modify the register value*/
    newRegVal = (registerVal & ~mask) | fieldVal;

    /* write to the register with modified value*/
    ADI_EXPECT(adi_fpga9001_RegisterWrite, device, addr, newRegVal);
