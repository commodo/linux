...
//        }
//    }
    adi_fpga9001_RegisterWrite(device, addr, regVal); 
    ADI_ERROR_RETURN(device->common.error.newAction);  
    return recoveryAction;
}
...
//       // recoveryAction = adi_fpga9001_hal_SpiByteRead(device, addr, regVal);
    ...
    //int32_t adi_fpga9001_RegisterRead(adi_fpga9001_Device_t *device, uint32_t offset, uint32_t *data)
    //ToDo replace theis with fieldRead.

    adi_fpga9001_RegisterRead(device, addr, regVal);   
    ADI_ERROR_RETURN(device->common.error.newAction);
//    }
    return recoveryAction;
}
...
//        {
            ...
            recoveryAction = adi_fpga9001_RegisterRead(device, addr, &registerVal);
            ADI_ERROR_RETURN(device->common.error.newAction);
            ...
            recoveryAction = adi_fpga9001_RegisterWrite(device, addr, newRegVal);
            ADI_ERROR_RETURN(device->common.error.newAction);
//        }
//    }
    return recoveryAction;
...
//        /* Implements immediate spi transaction call only used when caching is turned off. */
        ...
        recoveryAction = adi_fpga9001_RegisterFieldRead(device, addr, fieldVal, mask, startBit);
