 ...
 **/
static void adrv9001_LoadTwoBytes(uint32_t *offset, uint8_t cfgData[], const uint16_t data)
{
 ...
 **/
static void adrv9001_LoadFourBytes(uint32_t *offset, uint8_t cfgData[], const uint32_t data)
{
    uint32_t tempOffset = *offset;
 ...
 **/
static void adrv9001_LoadEightBytes(uint32_t *offset, uint8_t cfgData[], const uint64_t data)
{
    uint32_t tempOffset = *offset;
...
*/
static uint32_t adrv9001_LoadTxByte(adi_adrv9001_Device_t *device, 
                                    uint32_t *offset, 
                                    uint8_t cfgData[], 
                                    const adi_adrv9001_TxProfile_t *txProfile)
{
    ...
    /* txInterfaceSampleRate_Hz is referred as txInterfaceSampleRate in FW code */
    ...
    if ((txProfile->outputSignaling != ADI_ADRV9001_TX_IQ) &&
        (txProfile->outputSignaling != ADI_ADRV9001_TX_IQ_FM_FSK) &&
        (txProfile->outputSignaling != ADI_ADRV9001_TX_DIRECT_FM_FSK))
    {
        ADI_ERROR_REPORT(&device->common,
                         ADI_COMMON_ERRSRC_API,
                         ADI_COMMON_ERR_INV_PARAM,
                         ADI_COMMON_ACT_ERR_CHECK_PARAM,
                         txProfile->outputSignaling,
                         "Invalid output signal type for Tx");
...
/* This function handles only pfirBank_e enum type */
/*
Transmitter Structure: pfirBank_e
Size : 1 Byte
...
*/
static uint32_t adrv9001_LoadPfirBank(adi_adrv9001_Device_t *device, adi_adrv9001_PfirBank_e bank, uint32_t *offset, uint8_t cfgData[])
{
    uint32_t recoveryAction = ADI_COMMON_ACT_NO_ACTION;
    uint32_t tempOffset = *offset;

    if ((bank != ADI_ADRV9001_PFIR_BANK_A) &&
        (bank != ADI_ADRV9001_PFIR_BANK_B) &&
        (bank != ADI_ADRV9001_PFIR_BANK_C) &&
        (bank != ADI_ADRV9001_PFIR_BANK_D))
    {
        ADI_ERROR_REPORT(&device->common,
                         ADI_COMMON_ERRSRC_API,
                         ADI_COMMON_ERR_INV_PARAM,
                         ADI_COMMON_ACT_ERR_CHECK_PARAM,
                         bank,
                         "Invalid pre-processor block PFIR bank selection");
        ADI_ERROR_RETURN(device->common.error.newAction);
    }
    cfgData[tempOffset++] = (uint8_t)(bank & 0xFF);
...
*/
static uint32_t adrv9001_LoadTxPreProcBlockConfig(adi_adrv9001_Device_t *device, uint32_t *offset, uint8_t cfgData[], const adi_adrv9001_TxPreProc_t *txPreProc)
{
    uint32_t recoveryAction = ADI_COMMON_ACT_NO_ACTION;
    uint32_t tempOffset = *offset;
    ...
    if ( ( txPreProc->txPreProcMode != ADI_ADRV9001_TX_DP_PREPROC_MODE0 ) &&
         ( txPreProc->txPreProcMode != ADI_ADRV9001_TX_DP_PREPROC_MODE1 ) &&
         ( txPreProc->txPreProcMode != ADI_ADRV9001_TX_DP_PREPROC_MODE2 ) &&
         ( txPreProc->txPreProcMode != ADI_ADRV9001_TX_DP_PREPROC_MODE3 ) )
    {
        ADI_ERROR_REPORT(&device->common,
                         ADI_COMMON_ERRSRC_API,
                         ADI_COMMON_ERR_INV_PARAM,
                         ADI_COMMON_ACT_ERR_CHECK_PARAM,
                         txPreProc->txPreProcMode,
                         "Invalid Tx pre-processor block configuration mode");
        ADI_ERROR_RETURN(device->common.error.newAction);
    }
    cfgData[tempOffset++] = (uint8_t)(txPreProc->txPreProcMode & 0xFF);

    recoveryAction = adrv9001_LoadPfirBank(device, txPreProc->txPreProcWbNbPfirIBankSel, &tempOffset, cfgData);
    ADI_ERROR_REPORT(&device->common, ADI_COMMON_ERRSRC_API, ADI_COMMON_ERR_API_FAIL, recoveryAction, NULL, "Error from adrv9001_LoadPfirBank() for PFIR bank settings");
    ADI_ERROR_RETURN(device->common.error.newAction);

    recoveryAction = adrv9001_LoadPfirBank(device, txPreProc->txPreProcWbNbPfirQBankSel, &tempOffset, cfgData);
    ADI_ERROR_REPORT(&device->common, ADI_COMMON_ERRSRC_API, ADI_COMMON_ERR_API_FAIL, recoveryAction, NULL, "Error from adrv9001_LoadPfirBank() for PFIR bank settings");
    ADI_ERROR_RETURN(device->common.error.newAction);
...
*/
static uint32_t adrv9001_LoadTxIqdmDucBlockConfig(adi_adrv9001_Device_t *device, uint32_t *offset, uint8_t cfgData[], const adi_adrv9001_TxIqdmDuc_t *iqdmDuc)
{
    uint32_t recoveryAction = ADI_COMMON_ACT_NO_ACTION;
    uint32_t tempOffset = *offset;

    if ((iqdmDuc->iqdmDucMode != ADI_ADRV9001_TX_DP_IQDMDUC_MODE0) &&
        (iqdmDuc->iqdmDucMode != ADI_ADRV9001_TX_DP_IQDMDUC_MODE1) &&
        (iqdmDuc->iqdmDucMode != ADI_ADRV9001_TX_DP_IQDMDUC_MODE2))
    {
        ADI_ERROR_REPORT(&device->common,
            ADI_COMMON_ERRSRC_API,
            ADI_COMMON_ERR_INV_PARAM,
            ADI_COMMON_ACT_ERR_CHECK_PARAM,
            iqdmDuc->iqdmDucMode,
            "Invalid Tx Data Path IQDMDUC Mode");
        ADI_ERROR_RETURN(device->common.error.newAction);
    }
...
*/
static uint32_t adrv9001_LoadTxDataPathProfileConfig(adi_adrv9001_Device_t *device, uint32_t *offset, uint8_t cfgData[], const adi_adrv9001_TxDpProfile_t *txDpProfile)
{
    uint32_t recoveryAction = ADI_COMMON_ACT_NO_ACTION;
    uint32_t tempOffset = *offset;

    /* This function handles only txPreProc_t struct in txDpProfile_t struct */
    recoveryAction = adrv9001_LoadTxPreProcBlockConfig(device, &tempOffset, cfgData, &(txDpProfile->txPreProc));
    ADI_ERROR_REPORT(&device->common, ADI_COMMON_ERRSRC_API, ADI_COMMON_ERR_API_FAIL, recoveryAction, NULL, "Error from adrv9001_LoadTxPreProcBlockConfig()");
    ADI_ERROR_RETURN(device->common.error.newAction);
    ...
    /* This function handles only txIqdmDuc_t struct in txDpProfile_t struct */
    recoveryAction = adrv9001_LoadTxIqdmDucBlockConfig(device, &tempOffset, cfgData, &(txDpProfile->txIqdmDuc));
    ADI_ERROR_REPORT(&device->common, ADI_COMMON_ERRSRC_API, ADI_COMMON_ERR_API_FAIL, recoveryAction, NULL, "Error from adrv9001_LoadTxIqdmDucBlockConfig()");
    ADI_ERROR_RETURN(device->common.error.newAction);
...
*/
static uint32_t adrv9001_LoadSsiConfig(adi_adrv9001_Device_t *device, uint32_t *offset, uint8_t cfgData[], const adi_adrv9001_SsiConfig_t *ssiConfig)
{
    uint32_t recoveryAction = ADI_COMMON_ACT_NO_ACTION;
    uint32_t tempOffset = *offset;

    if ( ( ssiConfig->ssiType != ADI_ADRV9001_SSI_TYPE_DISABLE ) &&
         ( ssiConfig->ssiType != ADI_ADRV9001_SSI_TYPE_CMOS ) &&
         ( ssiConfig->ssiType != ADI_ADRV9001_SSI_TYPE_LVDS ) )
    {
        ADI_ERROR_REPORT(&device->common,
            ADI_COMMON_ERRSRC_API,
            ADI_COMMON_ERR_INV_PARAM,
    ...
    if ( ( ssiConfig->ssiDataFormatSel != ADI_ADRV9001_SSI_FORMAT_2_BIT_SYMBOL_DATA ) &&
         ( ssiConfig->ssiDataFormatSel != ADI_ADRV9001_SSI_FORMAT_8_BIT_SYMBOL_DATA ) &&
         ( ssiConfig->ssiDataFormatSel != ADI_ADRV9001_SSI_FORMAT_16_BIT_SYMBOL_DATA ) &&
         ( ssiConfig->ssiDataFormatSel != ADI_ADRV9001_SSI_FORMAT_12_BIT_I_Q_DATA ) &&
         ( ssiConfig->ssiDataFormatSel != ADI_ADRV9001_SSI_FORMAT_16_BIT_I_Q_DATA ) )
    {
        ADI_ERROR_REPORT(&device->common,
            ADI_COMMON_ERRSRC_API,
            ADI_COMMON_ERR_INV_PARAM,
            ADI_COMMON_ACT_ERR_CHECK_PARAM,
            ssiConfig->ssiDataFormatSel,
            "Invalid SSI data format");
        ADI_ERROR_RETURN(device->common.error.newAction);
    }
    cfgData[tempOffset++] = (uint8_t)(ssiConfig->ssiDataFormatSel & 0xFF);

    if ( ( ssiConfig->numLaneSel != ADI_ADRV9001_SSI_1_LANE ) &&
         ( ssiConfig->numLaneSel != ADI_ADRV9001_SSI_2_LANE ) &&
         ( ssiConfig->numLaneSel != ADI_ADRV9001_SSI_4_LANE ) )
    {
        ADI_ERROR_REPORT(&device->common,
            ADI_COMMON_ERRSRC_API,
            ADI_COMMON_ERR_INV_PARAM,
            ADI_COMMON_ACT_ERR_CHECK_PARAM,
            ssiConfig->numLaneSel,
            "Invalid SSI number of lanes");
        ADI_ERROR_RETURN(device->common.error.newAction);
    }
    cfgData[tempOffset++] = (uint8_t)(ssiConfig->numLaneSel & 0xFF);

    if ( ( ssiConfig->strobeType != ADI_ADRV9001_SSI_SHORT_STROBE ) &&
         ( ssiConfig->strobeType != ADI_ADRV9001_SSI_LONG_STROBE  ) )
    {
        ADI_ERROR_REPORT(&device->common,
            ADI_COMMON_ERRSRC_API,
            ADI_COMMON_ERR_INV_PARAM,
            ADI_COMMON_ACT_ERR_CHECK_PARAM,
            ssiConfig->strobeType,
            "Invalid SSI strobe type");
        ADI_ERROR_RETURN(device->common.error.newAction);
    }
...
*/
static int32_t adrv9001_TxProfileConfigWrite(adi_adrv9001_Device_t *device, const adi_adrv9001_TxProfile_t *txProfile, uint8_t cfgData[], uint32_t *offset)
{
    int32_t recoveryAction = ADI_COMMON_ACT_NO_ACTION;
    uint32_t tempOffset = *offset;
    ...
    recoveryAction = adrv9001_LoadTxByte(device, &tempOffset, cfgData, txProfile);
    ADI_ERROR_REPORT(&device->common, ADI_COMMON_ERRSRC_API, ADI_COMMON_ERR_API_FAIL, recoveryAction, NULL, "Error from adrv9001_LoadTxByte()");
    ADI_ERROR_RETURN(device->common.error.newAction);

    /* Handles txDpProfile_t within TxConfig_t in adrv9001_LoadTxDataPathProfileConfig() */
    recoveryAction = adrv9001_LoadTxDataPathProfileConfig(device, &tempOffset, cfgData, &(txProfile->txDpProfile));
    ADI_ERROR_REPORT(&device->common, ADI_COMMON_ERRSRC_API, ADI_COMMON_ERR_API_FAIL, recoveryAction, NULL, "Error from adrv9001_LoadTxDataPathProfileConfig()");
    ADI_ERROR_RETURN(device->common.error.newAction);

    /* Handles ssiConfig_t within TxConfig_t in adrv9001_LoadSsiConfig() */
    recoveryAction = adrv9001_LoadSsiConfig(device, &tempOffset, cfgData, &(txProfile->txSsiConfig));
    ADI_ERROR_REPORT(&device->common, ADI_COMMON_ERRSRC_API, ADI_COMMON_ERR_API_FAIL, recoveryAction, NULL, "Error from adrv9001_LoadSsiConfig()");
    ADI_ERROR_RETURN(device->common.error.newAction);
...
uint8_t             padding[1u];           
*/
static uint32_t adrv9001_LoadRxByte(adi_adrv9001_Device_t *device, 
    uint32_t *offset, 
    uint8_t cfgData[], 
    const adi_adrv9001_RxProfile_t *rxProfile)
{
    ...
    if ((rxProfile->outputSignaling != ADI_ADRV9001_RX_IQ) &&
        (rxProfile->outputSignaling != ADI_ADRV9001_RX_FREQUENCY_DEVIATION) &&
        (rxProfile->outputSignaling != ADI_ADRV9001_RX_FM_SYMBOLS))
    {
        ADI_ERROR_REPORT(&device->common,
            ADI_COMMON_ERRSRC_API,
            ADI_COMMON_ERR_INV_PARAM,
            ADI_COMMON_ACT_ERR_CHECK_PARAM,
            rxProfile->outputSignaling,
            "Invalid output signal type");
        ADI_ERROR_RETURN(device->common.error.newAction);
    }
    ...
    if ( ( rxProfile->adcType != ADI_ADRV9001_ADC_LP ) &&
         ( rxProfile->adcType != ADI_ADRV9001_ADC_HP ) )
    {
        ADI_ERROR_REPORT(&device->common,
            ADI_COMMON_ERRSRC_API,
            ADI_COMMON_ERR_INV_PARAM,
...
*/
static uint32_t adrv9001_LoadRxSincHbTopConfig(adi_adrv9001_Device_t *device, uint32_t *offset, uint8_t cfgData[], const adi_adrv9001_RxSincHbTop_t *rxSincHBTop)
{
    uint32_t recoveryAction = ADI_COMMON_ACT_NO_ACTION;
    uint32_t tempOffset = *offset;
    uint32_t count = 0;

    if ( ( rxSincHBTop->sincGainMux != ADI_ADRV9001_RX_SINC_GAIN_MUX_0_DB      ) &&
         ( rxSincHBTop->sincGainMux != ADI_ADRV9001_RX_SINC_GAIN_MUX_6_DB      ) &&
         ( rxSincHBTop->sincGainMux != ADI_ADRV9001_RX_SINC_GAIN_MUX_12_DB     ) &&
         ( rxSincHBTop->sincGainMux != ADI_ADRV9001_RX_SINC_GAIN_MUX_NEG_6_DB  ) &&
         ( rxSincHBTop->sincGainMux != ADI_ADRV9001_RX_SINC_GAIN_MUX_NEG_12_DB ) )
    {
        ADI_ERROR_REPORT(&device->common,
            ADI_COMMON_ERRSRC_API,
            ADI_COMMON_ERR_INV_PARAM,
            ADI_COMMON_ACT_ERR_CHECK_PARAM,
            rxSincHBTop->sincGainMux,
            "Invalid Sinc gain setting");
        ADI_ERROR_RETURN(device->common.error.newAction);
    }

    cfgData[tempOffset++] = (uint8_t)(rxSincHBTop->sincGainMux & 0xFF);

    if ( ( rxSincHBTop->sincMux != ADI_ADRV9001_RX_SINC_MUX5_OUTPUT_ZERO  ) &&
         ( rxSincHBTop->sincMux != ADI_ADRV9001_RX_SINC_MUX5_OUTPUT_SINC3 ) &&
         ( rxSincHBTop->sincMux != ADI_ADRV9001_RX_SINC_MUX5_OUTPUT_SINC4 ) &&
         ( rxSincHBTop->sincMux != ADI_ADRV9001_RX_SINC_MUX5_OUTPUT_SINC6 ) )
    {
        ADI_ERROR_REPORT(&device->common,
            ADI_COMMON_ERRSRC_API,
            ADI_COMMON_ERR_INV_PARAM,
            ADI_COMMON_ACT_ERR_CHECK_PARAM,
            rxSincHBTop->sincMux,
            "Invalid Sinc Mux selection");
        ADI_ERROR_RETURN(device->common.error.newAction);
    }

    cfgData[tempOffset++] = (uint8_t)(rxSincHBTop->sincMux & 0xFF);

    if ( ( rxSincHBTop->hbMux != ADI_ADRV9001_RX_HB_MUX_OUTPUT_ZERO) &&
         ( rxSincHBTop->hbMux != ADI_ADRV9001_RX_HB_MUX_OUTPUT_HB0 ) &&
         ( rxSincHBTop->hbMux != ADI_ADRV9001_RX_HB_MUX_OUTPUT_HB2 ) &&
         ( rxSincHBTop->hbMux != ADI_ADRV9001_RX_HB_MUX_OUTPUT_HB1 ) )
    {
        ADI_ERROR_REPORT(&device->common,
            ADI_COMMON_ERRSRC_API,
            ADI_COMMON_ERR_INV_PARAM,
...
uint8_t            padding[1u];
*/
static uint32_t adrv9001_LoadRxDpInFifoConfig(adi_adrv9001_Device_t *device, uint32_t *offset, uint8_t cfgData[], const adi_adrv9001_RxDpInFifoConfig_t *dpInFifo)
{
    uint32_t recoveryAction = ADI_COMMON_ACT_NO_ACTION;
    uint32_t tempOffset = *offset;

    if ( ( dpInFifo->dpInFifoTestDataSel != ADI_ADRV9001_DP_IN_FIFO_INPUT_DP_SEL ) &&
         ( dpInFifo->dpInFifoTestDataSel != ADI_ADRV9001_DP_IN_FIFO_INPUT_SPI_SEL) )
    {
        ADI_ERROR_REPORT(&device->common,
            ADI_COMMON_ERRSRC_API,
            ADI_COMMON_ERR_INV_PARAM,
            ADI_COMMON_ACT_ERR_CHECK_PARAM,
            dpInFifo->dpInFifoTestDataSel,
            "Invalid RX DP FIFO input data selection");
        ADI_ERROR_RETURN(device->common.error.newAction);
    }
    ...
    if ( ( dpInFifo->dpInFifoMode != ADI_ADRV9001_DP_IN_FIFO_MODE_DETECTING) &&
         ( dpInFifo->dpInFifoMode != ADI_ADRV9001_DP_IN_FIFO_MODE_DETECTED ) )
    {
        ADI_ERROR_REPORT(&device->common,
            ADI_COMMON_ERRSRC_API,
            ADI_COMMON_ERR_INV_PARAM,
            ADI_COMMON_ACT_ERR_CHECK_PARAM,
            dpInFifo->dpInFifoMode,
            "Invalid RX DP FIFO input mode selection");
        ADI_ERROR_RETURN(device->common.error.newAction);
    }
...
*/
static uint32_t adrv9001_LoadRxNbNcoConfig(adi_adrv9001_Device_t *device, uint32_t *offset, uint8_t cfgData[], const adi_adrv9001_RxNbNcoConfig_t *rxNbNco)
{
    uint32_t recoveryAction = ADI_COMMON_ACT_NO_ACTION;
    uint32_t tempOffset = *offset;
...
uint8_t            padding[1u];
*/
static uint32_t adrv9001_LoadRxWbNbCompPfirConfig(adi_adrv9001_Device_t *device, uint32_t *offset, uint8_t cfgData[], const adi_adrv9001_RxWbNbCompPFir_t *rxWbNbCompPFir)
{
    uint32_t recoveryAction = ADI_COMMON_ACT_NO_ACTION;
    uint32_t tempOffset = *offset;

    if ( ( rxWbNbCompPFir->rxWbNbCompPFirInMuxSel != ADI_ADRV9001_RP_FIR_IN_MUX_INT_IN ) &&
         ( rxWbNbCompPFir->rxWbNbCompPFirInMuxSel != ADI_ADRV9001_RP_FIR_IN_MUX_EXT_IN ) )
    {
        ADI_ERROR_REPORT(&device->common,
            ADI_COMMON_ERRSRC_API,
            ADI_COMMON_ERR_INV_PARAM,
            ADI_COMMON_ACT_ERR_CHECK_PARAM,
            rxWbNbCompPFir->rxWbNbCompPFirInMuxSel,
            "Invalid RX WbNb Compensation PFIR input selection");
        ADI_ERROR_RETURN(device->common.error.newAction);
    }
    ...
    recoveryAction = adrv9001_LoadPfirBank(device, rxWbNbCompPFir->bankSel, &tempOffset, cfgData);
    ADI_ERROR_REPORT(&device->common, ADI_COMMON_ERRSRC_API, ADI_COMMON_ERR_API_FAIL, recoveryAction, NULL, "Error from adrv9001_LoadPfirBank() for PFIR bank settings");
    ADI_ERROR_RETURN(device->common.error.newAction);
...
dpArmSel_e         dpArmSel;
*/
static uint32_t adrv9001_LoadRxNbDemConfig(adi_adrv9001_Device_t *device, uint32_t *offset, uint8_t cfgData[], const adi_adrv9001_RxNbDemConfig_t *rxNbDem)
{
    uint32_t recoveryAction = ADI_COMMON_ACT_NO_ACTION;
    uint32_t tempOffset = *offset;

    /* This function handles only dpInFifoConfig_t struct in rxnbDemConfig_t struct */
    recoveryAction = adrv9001_LoadRxDpInFifoConfig(device, &tempOffset, cfgData, &(rxNbDem->dpInFifo));
    ADI_ERROR_REPORT(&device->common, ADI_COMMON_ERRSRC_API, ADI_COMMON_ERR_API_FAIL, recoveryAction, NULL, "Error from adrv9001_LoadRxDpInFifoConfig()");
    ADI_ERROR_RETURN(device->common.error.newAction);

    /* This function handles only rxNbNcoConfig_t struct in rxnbDemConfig_t struct */
    recoveryAction = adrv9001_LoadRxNbNcoConfig(device, &tempOffset, cfgData, &(rxNbDem->rxNbNco));
    ADI_ERROR_REPORT(&device->common, ADI_COMMON_ERRSRC_API, ADI_COMMON_ERR_API_FAIL, recoveryAction, NULL, "Error from adrv9001_LoadRxNbNcoConfig()");
    ADI_ERROR_RETURN(device->common.error.newAction);

    /* This function handles only rxWbNbCompPFir_t struct in rxnbDemConfig_t struct */
    recoveryAction = adrv9001_LoadRxWbNbCompPfirConfig(device, &tempOffset, cfgData, &(rxNbDem->rxWbNbCompPFir));
    ADI_ERROR_REPORT(&device->common, ADI_COMMON_ERRSRC_API, ADI_COMMON_ERR_API_FAIL, recoveryAction, NULL, "Error from adrv9001_LoadRxWbNbCompPfirConfig()");
    ADI_ERROR_RETURN(device->common.error.newAction);
    ...
    if ( ( rxNbDem->gsOutMuxSel != ADI_ADRV9001_GS_OUT_MUX_GS_OUT ) &&
         ( rxNbDem->gsOutMuxSel != ADI_ADRV9001_GS_OUT_MUX_BYPASS) )
    {
        ADI_ERROR_REPORT(&device->common,
            ADI_COMMON_ERRSRC_API,
            ADI_COMMON_ERR_INV_PARAM,
            ADI_COMMON_ACT_ERR_CHECK_PARAM,
            rxNbDem->gsOutMuxSel,
            "Invalid GS out Mux selection");
        ADI_ERROR_RETURN(device->common.error.newAction);
    }

    cfgData[tempOffset++] = (uint8_t)(rxNbDem->gsOutMuxSel & 0xFF);

    if ( ( rxNbDem->rxOutSel != ADI_ADRV9001_RX_OUT_IQ_SEL) &&
         ( rxNbDem->rxOutSel != ADI_ADRV9001_RX_OUT_FMDM_SEL) &&
         ( rxNbDem->rxOutSel != ADI_ADRV9001_RX_OUT_SEL_PFIR) )
    {
        ADI_ERROR_REPORT(&device->common,
            ADI_COMMON_ERRSRC_API,
            ADI_COMMON_ERR_INV_PARAM,
            ADI_COMMON_ACT_ERR_CHECK_PARAM,
            rxNbDem->rxOutSel,
            "Invalid RX output selection");
        ADI_ERROR_RETURN(device->common.error.newAction);
    }

    cfgData[tempOffset++] = (uint8_t)(rxNbDem->rxOutSel & 0xFF);

    if ( ( rxNbDem->rxRoundMode != ADI_ADRV9001_RX_ROUNDMODE_IQ ) &&
         ( rxNbDem->rxRoundMode != ADI_ADRV9001_RX_ROUNDMODE_I  ) )
    {
        ADI_ERROR_REPORT(&device->common,
            ADI_COMMON_ERRSRC_API,
            ADI_COMMON_ERR_INV_PARAM,
            ADI_COMMON_ACT_ERR_CHECK_PARAM,
            rxNbDem->rxRoundMode,
            "Invalid RX round mode");
        ADI_ERROR_RETURN(device->common.error.newAction);
    }

    cfgData[tempOffset++] = (uint8_t)(rxNbDem->rxRoundMode & 0xFF);

    if ( ( rxNbDem->dpArmSel != ADI_ADRV9001_DP_SEL ) &&
         ( rxNbDem->dpArmSel != ADI_ADRV9001_ARM_SEL ) )
    {
        ADI_ERROR_REPORT(&device->common,
            ADI_COMMON_ERRSRC_API,
            ADI_COMMON_ERR_INV_PARAM,
...
*/
static uint32_t adrv9001_LoadRxDataPathProfileConfig(adi_adrv9001_Device_t *device, uint32_t *offset, uint8_t cfgData[], const adi_adrv9001_RxDpProfile_t *rxDpProfile)
{
    uint32_t recoveryAction = ADI_COMMON_ACT_NO_ACTION;
    uint32_t tempOffset = *offset;
    ...
    /* This function handles only rxSincHbTop_t struct in rxDpProfile_t struct */
    recoveryAction = adrv9001_LoadRxSincHbTopConfig(device, &tempOffset, cfgData, &(rxDpProfile->rxSincHBTop));
    ADI_ERROR_REPORT(&device->common, ADI_COMMON_ERRSRC_API, ADI_COMMON_ERR_API_FAIL, recoveryAction, NULL, "Error from adrv9001_LoadRxSincHbTopConfig()");
    ADI_ERROR_RETURN(device->common.error.newAction);

    /* This function handles only rxNbDemConfig_t struct in rxDpProfile_t struct */
    recoveryAction = adrv9001_LoadRxNbDemConfig(device, &tempOffset, cfgData, &(rxDpProfile->rxNbDem));
    ADI_ERROR_REPORT(&device->common, ADI_COMMON_ERRSRC_API, ADI_COMMON_ERR_API_FAIL, recoveryAction, NULL, "Error from adrv9001_LoadRxNbDemConfig()");
    ADI_ERROR_RETURN(device->common.error.newAction);
...
*/
static int32_t adrv9001_RxProfileConfigWrite(adi_adrv9001_Device_t *device, const adi_adrv9001_RxChannelCfg_t *rxChannelCfg, uint8_t cfgData[], uint32_t *offset)
{
    int32_t recoveryAction = ADI_COMMON_ACT_NO_ACTION;
    uint32_t tempOffset = *offset;
    ...
    recoveryAction = adrv9001_LoadRxByte(device, &tempOffset, cfgData, &(rxChannelCfg->profile));
    ADI_ERROR_REPORT(&device->common, ADI_COMMON_ERRSRC_API, ADI_COMMON_ERR_API_FAIL, recoveryAction, NULL, "Error from adrv9001_LoadRxByte()");
    ADI_ERROR_RETURN(device->common.error.newAction);

    /* Handles rxDpProfile_t within RxConfig_t in adrv9001_LoadRxDataPathProfileConfig() */
    recoveryAction = adrv9001_LoadRxDataPathProfileConfig(device, &tempOffset, cfgData, &(rxChannelCfg->profile.rxDpProfile));
    ADI_ERROR_REPORT(&device->common, ADI_COMMON_ERRSRC_API, ADI_COMMON_ERR_API_FAIL, recoveryAction, NULL, "Error from adrv9001_LoadRxDataPathProfileConfig()");
    ADI_ERROR_RETURN(device->common.error.newAction);

    /* Handles ssiConfig_t within RxConfig_t in adrv9001_LoadSsiConfig() */
    recoveryAction = adrv9001_LoadSsiConfig(device, &tempOffset, cfgData, &(rxChannelCfg->profile.rxSsiConfig));
    ADI_ERROR_REPORT(&device->common, ADI_COMMON_ERRSRC_API, ADI_COMMON_ERR_API_FAIL, recoveryAction, NULL, "Error from adrv9001_LoadSsiConfig()");
    ADI_ERROR_RETURN(device->common.error.newAction);
...
*/
static uint32_t adrv9001_DeviceSysConfigWrite(adi_adrv9001_Device_t *device, const adi_adrv9001_DeviceSysConfig_t *sysConfig, uint8_t cfgData[], uint32_t *offset)
{
    int32_t recoveryAction = ADI_COMMON_ACT_NO_ACTION;
    uint32_t tempOffset = *offset;
    uint32_t i = 0;

    if ( ( sysConfig->duplexMode != ADI_ADRV9001_TDD_MODE ) &&
         ( sysConfig->duplexMode != ADI_ADRV9001_FDD_MODE ) )
    {
        ADI_ERROR_REPORT(&device->common,
            ADI_COMMON_ERRSRC_API,
            ADI_COMMON_ERR_INV_PARAM,
            ADI_COMMON_ACT_ERR_CHECK_PARAM,
            sysConfig->duplexMode,
            "Invalid Duplex mode");
        ADI_ERROR_RETURN(device->common.error.newAction);
    }
    ...
    cfgData[tempOffset++] = sysConfig->monitorModeOn;

    cfgData[tempOffset++] = sysConfig->numDynamicProfile;

    cfgData[tempOffset++] = sysConfig->extMcsOn;

    if ((sysConfig->adcTypeMonitor != ADI_ADRV9001_ADC_LP) &&
        (sysConfig->adcTypeMonitor != ADI_ADRV9001_ADC_HP))
    {
        ADI_ERROR_REPORT(&device->common,
            ADI_COMMON_ERRSRC_API,
            ADI_COMMON_ERR_INV_PARAM,
            ADI_COMMON_ACT_ERR_CHECK_PARAM,
            sysConfig->ADI_ADRV9001_ADC_HP,
            "Invalid ADC mode");
        ADI_ERROR_RETURN(device->common.error.newAction);
    }
                                             ...
                                             uint32_t *checksum)
{
    ...
    for (i = 0; i < arraySize; i++)
    {
        ...
        recoveryAction = adrv9001_PfirRxNbPulShpFilterBank_Write(device, 
                                                                 ...
                                                                 &tempProfileAddr, 
                                                                 &tempChecksum);
        ADI_ERROR_REPORT(&device->common, ADI_COMMON_ERRSRC_API, ADI_COMMON_ERR_API_FAIL, recoveryAction, PfirWbNbFilterBankInfo[i], "Error from adrv9001_PfirRxNbPulShpFilterBank_Write()");
        ADI_ERROR_RETURN(device->common.error.newAction);
    }
...
int32_t adrv9001_DmaMemWrite(adi_adrv9001_Device_t *device, uint32_t address, const uint8_t data[], uint32_t byteCount)
{
    ...
    */
    ...
    if ((cacheEnable !=0) && (autoInc != 0))
    {
        ...
        /* FIXME: Vivek - Need to check if the commented code snippet can be deleted */
       // recoveryAction = adrv9001_DmaMemWriteCache(device, address, data, byteCount);
        //ADI_ERROR_RETURN(device->common.error.newAction);
        for (i = 0; i < byteCount; i++)
        {
...
int32_t adrv9001_ArmCmdErrorHandler(adi_adrv9001_Device_t *device, uint32_t detErr)
{
    ...
    if (armErrorFlag > 0)
    {
        ...
        ADI_ERROR_REPORT(&device->common, ADI_ADRV9001_SRC_ARMCMD, detErr, recoveryAction, detErr, errorString);
...
int32_t adrv9001_ArmProfileWrite(adi_adrv9001_Device_t *device, const adi_adrv9001_Init_t *init)
{
    ...
    static const uint32_t ADRV9001_PROFILE_MAX_SIZE = 1660;
    static const uint32_t ADRV9001_PROFILE_TX_SIZE = 156;
    static const uint32_t ADRV9001_PROFILE_RX_SIZE = 156;
    static const uint32_t ADRV9001_PROFILE_NOISE_CANCEL_SIZE = 28;

    static const uint32_t ADRV9001_RX1  = 0x0001; /*!< Rx1 channel enabled */
    static const uint32_t ADRV9001_RX2  = 0x0002; /*!< Rx2 channel enabled */
    static const uint32_t ADRV9001_TX1  = 0x0004; /*!< Tx1 channel enabled */
    static const uint32_t ADRV9001_TX2  = 0x0008; /*!< Tx2 channel enabled */
    static const uint32_t ADRV9001_ORX1 = 0x0010; /*!< ORx1 channel enabled */
    static const uint32_t ADRV9001_ORX2 = 0x0020; /*!< ORx2 channel enabled */
    static const uint32_t ADRV9001_ILB1 = 0x0040; /*!< ILB1 channel enabled */
    static const uint32_t ADRV9001_ILB2 = 0x0080; /*!< ILB2 channel enabled */
    static const uint32_t ADRV9001_ELB1 = 0x0100; /*!< ELB1 channel enabled */
    static const uint32_t ADRV9001_ELB2 = 0x0200; /*!< ELB2 channel enabled */
         ...
         ( init->clocks.rfPllPhaseSyncMode != ADI_ADRV9001_RFPLLMCS_INIT_AND_CONTTRACK ) )
    {
        ADI_ERROR_REPORT(&device->common,
            ...
            ADI_COMMON_ERR_INV_PARAM,
            ADI_COMMON_ACT_ERR_CHECK_PARAM,
            init->clocks.rfPllPhaseSyncMode,
            "Invalid RF PLL phase synchronization mode");
        ADI_ERROR_RETURN(device->common.error.newAction);
    }
    ...
    if ( ( init->clocks.rx1LoSelect != ADI_ADRV9001_LOSEL_LO1 ) &&
         ( init->clocks.rx1LoSelect != ADI_ADRV9001_LOSEL_LO2 ) )
    {
        ADI_ERROR_REPORT(&device->common,
            ADI_COMMON_ERRSRC_API,
            ADI_COMMON_ERR_INV_PARAM,
            ADI_COMMON_ACT_ERR_CHECK_PARAM,
            init->clocks.rx1LoSelect,
            "Invalid LO Select for Rx1. Valid values are LO1(1) and LO2(2)");
        ADI_ERROR_RETURN(device->common.error.newAction);
    }
    if ( ( init->clocks.rx2LoSelect != ADI_ADRV9001_LOSEL_LO1 ) &&
         ( init->clocks.rx2LoSelect != ADI_ADRV9001_LOSEL_LO2 ) )
    {
        ADI_ERROR_REPORT(&device->common,
            ADI_COMMON_ERRSRC_API,
            ADI_COMMON_ERR_INV_PARAM,
            ADI_COMMON_ACT_ERR_CHECK_PARAM,
            init->clocks.rx2LoSelect,
            "Invalid LO Select for Rx2. Valid values are LO1(1) and LO2(2)");
        ADI_ERROR_RETURN(device->common.error.newAction);
    }
    if ( ( init->clocks.tx1LoSelect != ADI_ADRV9001_LOSEL_LO1 ) &&
         ( init->clocks.tx1LoSelect != ADI_ADRV9001_LOSEL_LO2 ) )
    {
        ADI_ERROR_REPORT(&device->common,
            ADI_COMMON_ERRSRC_API,
            ADI_COMMON_ERR_INV_PARAM,
            ADI_COMMON_ACT_ERR_CHECK_PARAM,
            init->clocks.tx1LoSelect,
            "Invalid LO Select for Tx1. Valid values are LO1(1) and LO2(2)");
        ADI_ERROR_RETURN(device->common.error.newAction);
    }
    if ( ( init->clocks.tx2LoSelect != ADI_ADRV9001_LOSEL_LO1 ) &&
         ( init->clocks.tx2LoSelect != ADI_ADRV9001_LOSEL_LO2 ) )
    {
        ADI_ERROR_REPORT(&device->common,
            ADI_COMMON_ERRSRC_API,
            ADI_COMMON_ERR_INV_PARAM,
            ADI_COMMON_ACT_ERR_CHECK_PARAM,
            init->clocks.tx2LoSelect,
            "Invalid LO Select for Tx2. Valid values are LO1(1) and LO2(2)");
        ADI_ERROR_RETURN(device->common.error.newAction);
    }
    ...
    if ((init->clocks.rx1LoDivMode != ADI_ADRV9001_LO_DIV_MODE_HIGH_PERFORMANCE) &&
        ( init->clocks.rx1LoDivMode != ADI_ADRV9001_LO_DIV_MODE_LOW_POWER ))
    {
        ADI_ERROR_REPORT(&device->common,
            ADI_COMMON_ERRSRC_API,
            ADI_COMMON_ERR_INV_PARAM,
            ADI_COMMON_ACT_ERR_CHECK_PARAM,
            init->clocks.rx1LoDivMode,
            "Invalid RX1 LO divider mode Select. Valid values are BEST_PHASE_NOISE(0) and BEST_POWER_SAVING(1)");
        ADI_ERROR_RETURN(device->common.error.newAction);
    }
    if ((init->clocks.rx2LoDivMode != ADI_ADRV9001_LO_DIV_MODE_HIGH_PERFORMANCE) &&
        (init->clocks.rx2LoDivMode != ADI_ADRV9001_LO_DIV_MODE_LOW_POWER))
    {
        ADI_ERROR_REPORT(&device->common,
            ADI_COMMON_ERRSRC_API,
            ADI_COMMON_ERR_INV_PARAM,
            ADI_COMMON_ACT_ERR_CHECK_PARAM,
            init->clocks.rx2LoDivMode,
            "Invalid RX2 LO divider mode Select. Valid values are BEST_PHASE_NOISE(0) and BEST_POWER_SAVING(1)");
        ADI_ERROR_RETURN(device->common.error.newAction);
    }
    if ((init->clocks.tx1LoDivMode != ADI_ADRV9001_LO_DIV_MODE_HIGH_PERFORMANCE) &&
        (init->clocks.tx1LoDivMode != ADI_ADRV9001_LO_DIV_MODE_LOW_POWER))
    {
        ADI_ERROR_REPORT(&device->common,
            ADI_COMMON_ERRSRC_API,
            ADI_COMMON_ERR_INV_PARAM,
            ADI_COMMON_ACT_ERR_CHECK_PARAM,
            init->clocks.tx1LoDivMode,
            "Invalid TX1 LO divider mode Select. Valid values are BEST_PHASE_NOISE(0) and BEST_POWER_SAVING(1)");
        ADI_ERROR_RETURN(device->common.error.newAction);
    }
    if ((init->clocks.tx2LoDivMode != ADI_ADRV9001_LO_DIV_MODE_HIGH_PERFORMANCE) &&
        (init->clocks.tx2LoDivMode != ADI_ADRV9001_LO_DIV_MODE_LOW_POWER))
    {
        ADI_ERROR_REPORT(&device->common,
            ADI_COMMON_ERRSRC_API,
            ADI_COMMON_ERR_INV_PARAM,
            ADI_COMMON_ACT_ERR_CHECK_PARAM,
            init->clocks.tx2LoDivMode,
            "Invalid TX2 LO divider mode Select. Valid values are BEST_PHASE_NOISE(0) and BEST_POWER_SAVING(1)");
        ADI_ERROR_RETURN(device->common.error.newAction);
    }
    ...
    if ( ( init->clocks.loGen1Select != ADI_ADRV9001_LOGENPOWER_RFPLL_LDO ) &&
         ( init->clocks.loGen1Select != ADI_ADRV9001_LOGENPOWER_OFFCHIP ) )
    {
        ADI_ERROR_REPORT(&device->common,
            ADI_COMMON_ERRSRC_API,
            ADI_COMMON_ERR_INV_PARAM,
            ADI_COMMON_ACT_ERR_CHECK_PARAM,
            init->clocks.loGen1Select,
            "Invalid LoGenPower Selected for LoGen1. Valid values are RFPLL_LDO(1) and OffChip(2)");
        ADI_ERROR_RETURN(device->common.error.newAction);
    }
    if ( ( init->clocks.loGen2Select != ADI_ADRV9001_LOGENPOWER_RFPLL_LDO ) &&
         ( init->clocks.loGen2Select != ADI_ADRV9001_LOGENPOWER_OFFCHIP ) )
    {
        ADI_ERROR_REPORT(&device->common,
            ADI_COMMON_ERRSRC_API,
            ADI_COMMON_ERR_INV_PARAM,
            ADI_COMMON_ACT_ERR_CHECK_PARAM,
            init->clocks.loGen2Select,
            "Invalid LoGenPower Selected for LoGen2. Valid values are RFPLL_LDO(1) and OffChip(2)");
        ADI_ERROR_RETURN(device->common.error.newAction);
    }
    ...
    if ( ( init->clocks.rfPll1LoMode != ADI_ADRV9001_INT_LO1 ) &&
         ( init->clocks.rfPll1LoMode != ADI_ADRV9001_EXT_LO_OUTPUT ) &&
         ( init->clocks.rfPll1LoMode != ADI_ADRV9001_EXT_LO_INPUT1 )  &&
         ( init->clocks.rfPll1LoMode != ADI_ADRV9001_EXT_LO_INPUT2 ) )
    {
        ADI_ERROR_REPORT(&device->common,
            ADI_COMMON_ERRSRC_API,
            ADI_COMMON_ERR_INV_PARAM,
    ...
    if ( ( init->clocks.rfPll2LoMode != ADI_ADRV9001_INT_LO2 ) &&
         ( init->clocks.rfPll2LoMode != ADI_ADRV9001_EXT_LO_OUTPUT ) &&
         ( init->clocks.rfPll2LoMode != ADI_ADRV9001_EXT_LO_INPUT1 )  &&
         ( init->clocks.rfPll2LoMode != ADI_ADRV9001_EXT_LO_INPUT2 ) )
    {
        ADI_ERROR_REPORT(&device->common,
            ADI_COMMON_ERRSRC_API,
            ADI_COMMON_ERR_INV_PARAM,
    ...
    if ( ( init->clocks.ext1LoType != ADI_ADRV9001_EXT_LO_DIFFERENTIAL ) &&
         ( init->clocks.ext1LoType != ADI_ADRV9001_EXT_LO_SINGLE_ENDED ) )
    {
        ADI_ERROR_REPORT(&device->common,
            ADI_COMMON_ERRSRC_API,
            ADI_COMMON_ERR_INV_PARAM,
            ADI_COMMON_ACT_ERR_CHECK_PARAM,
            init->clocks.ext1LoType,
            "Invalid EXT1 LO type");
        ADI_ERROR_RETURN(device->common.error.newAction);
    }
    if ( ( init->clocks.ext2LoType != ADI_ADRV9001_EXT_LO_DIFFERENTIAL ) &&
         ( init->clocks.ext2LoType != ADI_ADRV9001_EXT_LO_SINGLE_ENDED ) )
    {
        ADI_ERROR_REPORT(&device->common,
            ADI_COMMON_ERRSRC_API,
            ADI_COMMON_ERR_INV_PARAM,
            ADI_COMMON_ACT_ERR_CHECK_PARAM,
            init->clocks.ext2LoType,
            "Invalid EXT2 LO type");
        ADI_ERROR_RETURN(device->common.error.newAction);
    }
    ...
    {
        if (ADRV9001_BF_EQUAL(init->tx.txInitChannelMask, ADI_CHANNEL_1))
        {
            armChannels |= ADRV9001_TX1;
        }
        if (ADRV9001_BF_EQUAL(init->tx.txInitChannelMask, ADI_CHANNEL_2))
        {
            armChannels |= ADRV9001_TX2;
        }
    }
    ...
    {
        if (ADRV9001_BF_EQUAL(init->rx.rxInitChannelMask, ADI_CHANNEL_1))
        {
            armChannels |= ADRV9001_RX1;
        }
        if (ADRV9001_BF_EQUAL(init->rx.rxInitChannelMask, ADI_CHANNEL_2))
        {
            armChannels |= ADRV9001_RX2;
        }
    }
    ...
    {
        if (ADRV9001_BF_EQUAL(init->rx.rxInitChannelMask, ADI_ADRV9001_ORX1))
        {
            armChannels |= ADRV9001_ORX1;
        }
        if (ADRV9001_BF_EQUAL(init->rx.rxInitChannelMask, ADI_ADRV9001_ORX2))
        {
            armChannels |= ADRV9001_ORX2;
        }
    }
    /* Update 'armChannels' with ILB1/2 channel information if the channel is enabled; No action is taken otherwise  */
    if (ADRV9001_BF_EQUAL(device->devStateInfo.profilesValid, ADI_ADRV9001_ORX_PROFILE_VALID))
    {
        if (ADRV9001_BF_EQUAL(init->rx.rxInitChannelMask, ADI_ADRV9001_ILB1))
        {
            armChannels |= ADRV9001_ILB1;
        }
        if (ADRV9001_BF_EQUAL(init->rx.rxInitChannelMask, ADI_ADRV9001_ILB2))
        {
            armChannels |= ADRV9001_ILB2;
        }
    }
    /* Update 'armChannels' with ELB1/2 channel information if the channel is enabled; No action is taken otherwise  */
    if (ADRV9001_BF_EQUAL(device->devStateInfo.profilesValid, ADI_ADRV9001_ORX_PROFILE_VALID))
    {
        if (ADRV9001_BF_EQUAL(init->rx.rxInitChannelMask, ADI_ADRV9001_ELB1))
        {
            armChannels |= ADRV9001_ELB1;
        }
        if (ADRV9001_BF_EQUAL(init->rx.rxInitChannelMask, ADI_ADRV9001_ELB2))
        {
            armChannels |= ADRV9001_ELB2;
        }
    }
    if ( ( init->clocks.rx1RfInputSel != ADI_ADRV9001_RX_A ) &&
         ( init->clocks.rx1RfInputSel != ADI_ADRV9001_RX_B ) )
    {
        ADI_ERROR_REPORT(&device->common,
            ADI_COMMON_ERRSRC_API,
            ADI_COMMON_ERR_INV_PARAM,
            ADI_COMMON_ACT_ERR_CHECK_PARAM,
            init->clocks.rx1RfInputSel,
            "Invalid Rx1 pin selection");
        ADI_ERROR_RETURN(device->common.error.newAction);
    }
    if ( ( init->clocks.rx2RfInputSel != ADI_ADRV9001_RX_A ) &&
         ( init->clocks.rx2RfInputSel != ADI_ADRV9001_RX_B ) )
    {
        ADI_ERROR_REPORT(&device->common,
            ADI_COMMON_ERRSRC_API,
            ADI_COMMON_ERR_INV_PARAM,
            ADI_COMMON_ACT_ERR_CHECK_PARAM,
            init->clocks.rx2RfInputSel,
            "Invalid Rx2 pin selection");
        ADI_ERROR_RETURN(device->common.error.newAction);
    }
    ...
    /* TX CONFIG 172 bytes per TX channel */
    for (i = 0; i < ADI_ADRV9001_MAX_TXCHANNELS; i++)
    {
        adrv9001_cfgDataSet(&cfgData[0], 0, ADRV9001_PROFILE_CHUNK_MAX);
        /* Check whether the Tx channel is valid and write '0' for invalid channel */
        if ( ADRV9001_BF_EQUAL( init->tx.txInitChannelMask, ( ADI_CHANNEL_1 << i ) ) )
        {
            recoveryAction = adrv9001_TxProfileConfigWrite(device, &(init->tx.txProfile[i]), &cfgData[0], &offset);
            ADI_ERROR_REPORT(&device->common, ADI_COMMON_ERRSRC_API, ADI_COMMON_ERR_API_FAIL, recoveryAction, NULL, "Error from adrv9001_TxProfileConfigWrite()");
            ADI_ERROR_RETURN(device->common.error.newAction);
        }
        else
        {
    ...
    /* RX CONFIG 164 bytes per channel */
    for (i = 0; i < ADI_ADRV9001_MAX_RXCHANNELS; i++)
    {
        adrv9001_cfgDataSet(&cfgData[0], 0, ADRV9001_PROFILE_CHUNK_MAX);
        /* Check whether the Rx channel is valid and write '0' for invalid channel */
        if ( ADRV9001_BF_EQUAL( init->rx.rxInitChannelMask, ( ADI_ADRV9001_RX1 << i ) ) )
        {
            recoveryAction = adrv9001_RxProfileConfigWrite(device, &(init->rx.rxChannelCfg[i]), &cfgData[0], &offset);
            ADI_ERROR_REPORT(&device->common, ADI_COMMON_ERRSRC_API, ADI_COMMON_ERR_API_FAIL, recoveryAction, NULL, "Error from adrv9001_RxProfileConfigWrite()");
            ADI_ERROR_RETURN(device->common.error.newAction);
        }
        else
        {
    ...
    recoveryAction = adrv9001_DeviceSysConfigWrite(device, &(init->sysConfig), &cfgData[0], &offset);
    ADI_ERROR_REPORT(&device->common, ADI_COMMON_ERRSRC_API, ADI_COMMON_ERR_API_FAIL, recoveryAction, NULL, "Error from adrv9001_DeviceSysConfigWrite()");
    ADI_ERROR_RETURN(device->common.error.newAction);
...
int32_t adrv9001_PfirProfilesWrite(adi_adrv9001_Device_t *device, const adi_adrv9001_Init_t *init)
{
    ...
    if (device->devStateInfo.pfirProfileAddr < ADRV9001_ADDR_DEVICE_PROFILE)
    {
        ADI_ERROR_REPORT(&device->common,
            ADI_COMMON_ERRSRC_API,
            ADI_COMMON_ERR_INV_PARAM,
    ...
    profileAddr = device->devStateInfo.pfirProfileAddr;
    ...
    if (profileAddr != (device->devStateInfo.pfirProfileAddr + ADRV9001_PROFILE_MAX_SIZE))
    {
        ADI_ERROR_REPORT(&device->common,
            ADI_COMMON_ERRSRC_API,
            ADI_COMMON_ERR_INV_PARAM,
