...
int32_t adrv9001_GpIntHandler(adi_adrv9001_Device_t *device, adi_adrv9001_gpIntStatus_t *gpIntStatus)
{
    ...
    /* FIXME: Vivek - Need review the recovery action for each of the interrupt source */
    ...
    uint32_t bfValue)
{
    ...
    /*FIXME: Vivek - Uncomment declaration of 'MAX_RANGE_UNSIGNED_32BIT' when 'ADRV9001_BITFIELD_VALUE_CHECK' is enabled */
...
#if ADRV9001_BITFIELD_VALUE_CHECK > 0
    /* Range check */
    if ((bfValue < 0) ||
...
#if ADRV9001_BITFIELD_ADDR_CHECK > 0
    /* Base Address check */
