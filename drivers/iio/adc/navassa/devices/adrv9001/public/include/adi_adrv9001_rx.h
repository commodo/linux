...
* \param[in] gainCtrlMode   Desired gain control mode
*
...
*        Slow Loop AGC, Hybrid mode for channels Rx1, Rx2, ORx1, ORx2, ILB1, ILB2, ELB1 and ELB2. 
*        This API is used to read back the current Rx gain ctrl mode setting.
*
* \param[in]  device    Pointer to the ADRV9001 device data structure
* \param[in]  channel   Rx channel for which gain control mode read back is requested
* \param[out] gainMode  pointer to adi_adrv9001_RxAgcMode_t structure which will be updated with the read back config
*
* \returns A code indicating success (ADI_COMMON_ACT_NO_ACTION) or the required action to recover
*/
...
*      running init cals.
*
* \param[in] device             Structure pointer to the ADRV9001 device data structure
 ...
 *
 * This function reads the gain index for the passed channel Rx1, Rx2, ORx1, ORx2
 *
 ...
 * \param[in] gainIndex The current gain table index the channel is set to
 *
 * \returns A code indicating success (ADI_COMMON_ACT_NO_ACTION) or the required action to recover
 */
int32_t adi_adrv9001_Rx_Gain_Get(adi_adrv9001_Device_t *device,
                                 adi_common_ChannelNumber_e channel,
                                 uint8_t *gainIndex);
...
*
* \pre This function may be called anytime after the device is fully initialized
*
...
*
* \pre This function can be called after the ARM has been initialized.
*
...
int32_t adi_adrv9001_Rx_InterfaceGain_Configure(adi_adrv9001_Device_t *device,
                                                adi_common_ChannelNumber_e channel,
                                                adrv9001_RxInterfaceGainCtrl_t *rxInterfaceGainConfig);

/**
* \brief Set the Rx interface gain
*
* \pre This function can be called after the ARM has been initialized.
* 'gainControlMode' in adrv9001_RxInterfaceGainCtrl_t struct must be ADI_ADRV9001_RX_INTERFACE_GAIN_CONTROL_MANUAL 
