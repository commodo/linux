...
{
    static const uint8_t OBJID_SYS_MISC = 0xFF;
    uint8_t objectId = 0;
    uint8_t errorCode = 0;

    ADI_API_ENTRY_PTR_EXPECT(device, action);

    ADI_EXPECT(adi_adrv9001_arm_SystemError_Get, device, &objectId, &errorCode);

    if (OBJID_SYS_MISC != objectId)
    {
        ADI_ERROR_REPORT(&device->common,
                         ...
                         NULL,
                         "Invalid object ID. Are you sure the ADRV9001 requested an MCS?");
    }
    *action = (adi_adrv9001_McsAction_e)errorCode;
