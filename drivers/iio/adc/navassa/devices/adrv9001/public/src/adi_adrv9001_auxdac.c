...
/* System header files */
    ...
    #line __LINE__ "adi_adrv9001_auxdac.c"
#endif
...
static int32_t adi_adrv9001_AuxDac_Configure_Validate(adi_adrv9001_Device_t *device,
                                      ...
                                      bool enable)
{
    ...
    uint8_t resourceAcquisitionStatus = false;
    uint8_t resourceReleaseStatus = false;
    int32_t sharedResourceArr[ADI_ADRV9001_MAX_AUXDACS] = { 0 };
    ...
    for (k = 0; k < ADI_ADRV9001_MAX_AUXDACS; k++)
    {
        if (auxDac == (adi_adrv9001_AuxDacs_e)(1 << k))
        {
    ...
    ADI_MSG_EXPECT("Error while releasing shared GPIO pins while disabling AuxDACs", 
                       adrv9001_SharedResourcesRelease,
                       device,
                       ADRV9001_SHARED_RESOURCE_GPIO_ANALOG,
                       &sharedResourceArr[0],
                       numSharedResources,
                       ADRV9001_FEATURE_AUX_DAC_OUT,
    ...
    if (resourceReleaseStatus == false)
    {
        ADI_ERROR_REPORT(&device->common,
                         ...
                         ADI_ADRV9001_ERR_SHARED_RESOURCE_RELEASE,
                         ADI_ADRV9001_ACT_ERR_RESET_GPIO,
                         resourceReleaseStatus,
                         "Unable to release GPIOs for AuxDACs while disabling AuxDACs ");
        ADI_ERROR_RETURN(device->common.error.newAction);
    }

    /* Configure Analog GPIO pin as input */
    ADI_MSG_EXPECT("Unable to set GPIO direction as input", 
                   adi_adrv9001_GpioAnalogInputDirSet, 
                   device, 
                   auxDac);
    ...
    if (enable > 0)
    {
        ...
        ADI_MSG_EXPECT("Error while acquiring shared GPIO pins while enabling AuxDACs", 
                       adrv9001_SharedResourcesAcquire,
                       device,
                       ADRV9001_SHARED_RESOURCE_GPIO_ANALOG,
                       &sharedResourceArr[0],
                       numSharedResources,
        ...
        if (resourceAcquisitionStatus == false)
        {
            ADI_ERROR_REPORT(&device->common,
                             ADI_COMMON_ERRSRC_API,
                             ADI_ADRV9001_ERR_SHARED_RESOURCE_ACQUIRE,
                             ADI_ADRV9001_ACT_ERR_RESET_GPIO,
                             resourceAcquisitionStatus,
        ...
        if ((recoveryAction = adi_adrv9001_GpioAnalogOutputDirSet(device, auxDac)) != ADI_COMMON_ACT_NO_ACTION)
        {
            ...
            ADI_MSG_EXPECT("Error while releasing shared GPIO pins", 
                           adrv9001_SharedResourcesRelease,
                           device,
                           ADRV9001_SHARED_RESOURCE_GPIO_ANALOG,
                           &sharedResourceArr[0],
