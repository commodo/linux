...
#include "adrv9001_reg_addr_macros.h"
    ...
    #line __LINE__ "adi_adrv9001_tx.c"
#endif
...
/* FIXME: Vivek - Commented for now; Will have to be uncommented when needed */
//#define ADI_ADRV9001_NUM_GPIOS_IN_TX_ATTEN_CTRL_PIN_FEATURE 2U
...
int32_t adi_adrv9001_Tx_Attenuation_Configure(adi_adrv9001_Device_t *device,
                                              ...
                                              adi_adrv9001_TxAttenuationConfig_t *config)
{
    uint8_t regTpcConfig = 0;

    adrv9001_BfNvsRegmapTxChanAddr_e txChannelBaseAddr = ADRV9001_BF_TX1_CORE;

    static const uint32_t ADDR_TX_TPC_CONFIG_OFFSET    = 0xD8;
    ...
    /* recoveryAction = adrv9001_TxTxAttenConfigBfSet(device, txChannelBaseAddr, config->txAttenStepSize); */
    regTpcConfig |= ADRV9001_BF_ENCODE(config->txAttenStepSize, 0x30, 4);

    /* Attenuation mode */
    ADI_EXPECT(adi_adrv9001_Tx_AttenuationMode_Set, device, channel, config->attenMode);
    //TODO: Is this necessary?
    {
        /* TODO: attenMode value for SPI2 does not map to this bitfield, need casing logic */
        regTpcConfig |= ADRV9001_BF_ENCODE(config->attenMode, 0x03, 0);

        ADRV9001_SPIWRITEBYTE(device, "TX_TPC_CONFIG", (txChannelBaseAddr + ADDR_TX_TPC_CONFIG_OFFSET), regTpcConfig);
    }
    ...
    device->devStateInfo.txAttenStepSize = config->txAttenStepSize;
                                            ...
                                            adi_adrv9001_TxAttenuationConfig_t *config)
{
    ...
    adi_adrv9001_TxAttenMode_e txModeRead = ADI_ADRV9001_TXATTEN_SPI_ATTEN_MODE;
...
{
    ADI_RANGE_CHECK(device, channel, ADI_CHANNEL_1, ADI_CHANNEL_2);
    ADI_RANGE_CHECK(device, mode, ADI_ADRV9001_TXATTEN_BYPASS_MODE, ADI_ADRV9001_TXATTEN_GPIO_MODE);
                                            ...
                                            adi_adrv9001_TxAttenMode_e *mode)
{
    ...
    *mode = (adi_adrv9001_TxAttenMode_e)regData;
                                                        ...
                                                        uint16_t attenuation_mdB)
{
    ...
    if (device->devStateInfo.outputSignaling[adi_channel_to_index(channel)] == ADI_ADRV9001_TX_DIRECT_FM_FSK)
    {
                                        ...
                                        uint16_t attenuation_mdB)
{
    uint16_t regData = 0;
    adrv9001_BfNvsRegmapTxChanAddr_e txChannelBaseAddr = ADRV9001_BF_TX1_CORE;
    adi_adrv9001_TxAttenMode_e attenMode = ADI_ADRV9001_TXATTEN_BYPASS_MODE;
    ...
    if (attenMode != ADI_ADRV9001_TXATTEN_SPI_ATTEN_MODE)
    {
        ADI_EXPECT(adi_adrv9001_Tx_AttenuationMode_Set, device, channel, ADI_ADRV9001_TXATTEN_SPI_ATTEN_MODE);
    }
    ...
    {
        /* TODO: Read current step size from register and remove from devStateInfo */
                                              ...
                                              uint16_t *numAttenIndicesRead)
{
    ...
    ADI_EXPECT(adrv9001_DmaMemRead, device, offset + stop, &cfgData[0], numTxAttenEntriesRead, 0);
                                       ...
                                       adi_adrv9001_PaRampCfg_t *paRampCfg)
{
    ...
    if (channel == ADI_CHANNEL_1)
    {
        ...
        paRampCfg->gpioSource = (adi_adrv9001_GpioPinSel_e)bfValue;
