...
int32_t zc706sd20_eeprom_FmcFru_Read(fru_data_t *fru)
{
    ...
    {
        _fru = parse_FRU(raw_input_data);
        free(raw_input_data);

        fru->Board_Area.language_code = _fru->Board_Area->language_code;
        fru->Board_Area.mfg_date = _fru->Board_Area->mfg_date;
        strncpy(fru->Board_Area.manufacturer, (char *)_fru->Board_Area->manufacturer + 1, 128);
        strncpy(fru->Board_Area.product_name, (char *)_fru->Board_Area->product_name + 1, 128);
        strncpy(fru->Board_Area.serial_number, (char *)_fru->Board_Area->serial_number + 1, 128);
        strncpy(fru->Board_Area.part_number, (char *)_fru->Board_Area->part_number + 1, 128);
        strncpy(fru->Board_Area.FRU_file_ID, (char *)_fru->Board_Area->FRU_file_ID + 1, 128);
        for (i = 0; i < 4; i++)
        {
            if (_fru->Board_Area->custom[i] && _fru->Board_Area->custom[i][0] & 0x3F)
            {
                ...
                {
                case 0:
                    strncpy(fru->Board_Area.pcb_rev, (char *)_fru->Board_Area->custom[i] + 2, 128);
                    break;
                case 1:
                    strncpy(fru->Board_Area.pcb_id, (char *)_fru->Board_Area->custom[i] + 2, 128);
                    break;
                case 2:
                    strncpy(fru->Board_Area.bom_rev, (char *)_fru->Board_Area->custom[i] + 2, 128);
                    break;
                case 3:
                    strncpy(fru->Board_Area.uses_lvds, (char *)_fru->Board_Area->custom[i] + 2, 128);
                    break;
                default:
                    /* Unknown field */
                    break;
