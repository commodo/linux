			       ...
			       unsigned int mode)
{
	...
	ret = ad9467_spi_write(st->spi, ADI_ADC_REG_CHAN_INDEX, 1 << chan);
	if (ret < 0)
		return ret;

	ret = ad9467_spi_write(st->spi, ADI_ADC_REG_TEST_IO, mode);
	if (ret < 0)
		return ret;

	ret = ad9467_spi_write(st->spi, ADI_ADC_REG_CHAN_INDEX, 0x3);
	if (ret < 0)
		return ret;

	ret = ad9467_spi_write(st->spi, ADI_ADC_REG_TRANSFER,
			       ADI_ADC_TRANSFER_SYNC);
	if (ret < 0)
		return ret;
...
static unsigned int ad9467_pnsel_to_testmode(enum axi_adc_pn_sel sel)
{
	switch (sel) {
	case AXI_ADC_PN9:
		return ADI_ADC_TESTMODE_PN9_SEQ;
	default:
		return ADI_ADC_TESTMODE_OFF;
	}
}

static int ad9467_pnsel_set(struct axi_adc_conv *conv, int chan,
			    enum axi_adc_pn_sel sel)
{
	struct ad9467_state *st = axi_adc_conv_priv(conv);
	unsigned int output_mode = st->output_mode;
	int ret, testmode;

	testmode = ad9467_pnsel_to_testmode(sel);

	if (testmode != ADI_ADC_TESTMODE_OFF)
		output_mode &= ~ADI_ADC_OUTPUT_MODE_TWOS_COMPLEMENT;

	ret = ad9467_outputmode_set(st->spi, output_mode);
	if (ret < 0)
		return ret;

	return ad9467_testmode_set(st, chan, testmode);
}
...
static int ad9467_preenable_setup(struct axi_adc_conv *conv)
{
	...
	ret = axi_adc_lvds_idelay_calibrate(conv);
	if (ret)
		return ret;
...
static int ad9467_probe(struct spi_device *spi)
{
	...
	conv->pnsel_set = ad9467_pnsel_set;
