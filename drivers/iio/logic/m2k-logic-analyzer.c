...
static void m2k_la_write(struct m2k_la_state *st, unsigned int reg,
	unsigned int val)
{
	writel_relaxed(val, st->regs + reg);
}

static unsigned int m2k_la_read(struct m2k_la_state *st, unsigned int reg)
{
	return readl_relaxed(st->regs + reg);
}

static void m2k_la_update(struct m2k_la_state *st, unsigned int reg,
			      unsigned int writeval, const unsigned int mask)
{
	unsigned int regval;

	regval = m2k_la_read(st, reg);
	regval &= ~mask;
	writeval &= mask;
	m2k_la_write(st, reg, writeval | regval);
}

static int m2k_la_read_raw(struct iio_dev *indio_dev,
	const struct iio_chan_spec *chan, int *val, int *val2, long info)
{
	struct m2k_la_state *st = iio_priv(indio_dev);
	...
	case IIO_CHAN_INFO_RAW:
		mutex_lock(&st->lock);
		if (st->io_sel & ch_mask)
			raw = m2k_la_read(st, M2K_LA_REG_GPI);
		else
			raw = st->gpo;
	...
	const struct iio_chan_spec *chan, int val, int val2, long info)
{
	...
	case IIO_CHAN_INFO_RAW:
		...
		if (val)
			...
			st->gpo &= ~ch_mask;
		m2k_la_write(st, M2K_LA_REG_GPO, st->gpo);
		mutex_unlock(&st->lock);
...
static int m2k_la_txrx_read_raw(struct iio_dev *indio_dev,
	const struct iio_chan_spec *chan, int *val, int *val2, long info)
{
	struct m2k_la_state *st = iio_device_get_drvdata(indio_dev);
	unsigned int div, reg;

	if (indio_dev->buffer->direction == IIO_DEVICE_DIRECTION_IN)
		reg = M2K_LA_REG_DIVIDER_LA;
	else
		reg = M2K_LA_REG_DIVIDER_PG;

	div = m2k_la_read(st, reg) + 1;

	*val = DIV_ROUND_CLOSEST(clk_get_rate(st->clk), div*10) * 10;
	...
	const struct iio_chan_spec *chan, int val, int val2, long info)
{
	...
	div = DIV_ROUND_UP(clk_get_rate(st->clk), val);
	if (indio_dev->buffer->direction == IIO_DEVICE_DIRECTION_IN)
		reg = M2K_LA_REG_DIVIDER_LA;
	else
		reg = M2K_LA_REG_DIVIDER_PG;

	m2k_la_write(st, reg, div - 1);

	return 0;
}

static void m2k_la_update_logic_mode(struct m2k_la_state *st)
{
	 ...
	 */
	if (st->trigger_num_enabled == 0)
		mode = M2K_LA_TRIGGER_LOGIC_MODE_AND;
	else
		mode = st->trigger_logic_mode;

	mask = m2k_la_read(st, M2K_LA_REG_TRIGGER_LOGIC_MODE);
	mask &= ~BIT(0);

	m2k_la_write(st, M2K_LA_REG_TRIGGER_LOGIC_MODE, mask | mode);
}
	...
	const struct iio_chan_spec *chan, unsigned int val)
{
	...
	mutex_lock(&st->lock);
	old = st->triggers[chan->address];
	if (old == val) {
		mutex_unlock(&st->lock);
		return 0;
	}

	st->triggers[chan->address] = val;
	if (old != 0) {
		old -= 1;
		st->trigger_regs[old] &= ~ch_mask;
		m2k_la_write(st, M2K_LA_REG_TRIGGER_ENABLE(old),
			st->trigger_regs[old]);
		st->trigger_num_enabled--;
	}
	if (val != 0) {
		val -= 1;
		st->trigger_regs[val] |= ch_mask;
		m2k_la_write(st, M2K_LA_REG_TRIGGER_ENABLE(val),
			st->trigger_regs[val]);
		st->trigger_num_enabled++;
	}

	m2k_la_update_logic_mode(st);
	...
	const struct iio_chan_spec *chan)
{
	...
	return st->triggers[chan->address];
}
	...
	const struct iio_chan_spec *chan, unsigned int val)
{
	...
	mask &= ~GENMASK(6, 4);
	val = val << 4;
	m2k_la_write(st, M2K_LA_REG_TRIGGER_LOGIC_MODE, mask | val);
	mutex_unlock(&st->lock);
	...
	const struct iio_chan_spec *chan)
{
	...
	val = m2k_la_read(st, M2K_LA_REG_TRIGGER_LOGIC_MODE);
	return (val >> 4);
}
	...
	size_t len)
{
	...
	mutex_lock(&st->lock);
	if (val < 0) {
		m2k_la_write(st, M2K_LA_REG_FIFO_DEPTH, -val);
		m2k_la_write(st, M2K_LA_REG_TRIGGER_DELAY, 0);
	} else {
		m2k_la_write(st, M2K_LA_REG_FIFO_DEPTH, 0);
		m2k_la_write(st, M2K_LA_REG_TRIGGER_DELAY, val);
	}
	st->trigger_delay = val;
	mutex_unlock(&st->lock);
	...
	size_t len)
{
	...
	m2k_la_write(st, M2K_LA_REG_TRIGGERED, val);
	...
	uintptr_t priv, const struct iio_chan_spec *chan, char *buf)
{
	...
	val = m2k_la_read(st, M2K_LA_REG_TRIGGERED) & 1;
	...
	const struct iio_chan_spec *chan, unsigned int val)
{
	...
	m2k_la_update(st, M2K_LA_REG_DATA_DELAY_CONFIG,
	...
	const struct iio_chan_spec *chan)
{
	...
	val = m2k_la_read(st, M2K_LA_REG_DATA_DELAY_CONFIG);
	...
	size_t len)
{
	...
	m2k_la_update(st, M2K_LA_REG_DATA_DELAY_CONFIG, val,
	...
	uintptr_t priv, const struct iio_chan_spec *chan, char *buf)
{
	...
	val = m2k_la_read(st, M2K_LA_REG_STREAMING) & 1;
	...
	size_t len)
{
	...
	m2k_la_write(st, M2K_LA_REG_STREAMING, val);
	...
	const struct iio_chan_spec *chan, unsigned int val)
{
	...
	mutex_lock(&st->lock);
	/* Read Trig SRC */
	trig_src = m2k_la_read(st, M2K_LA_REG_INSTRUMENT_TRIGGER);
	...
	if (trig_src) {
		...
		m2k_la_update(st, M2K_LA_REG_INSTRUMENT_TRIGGER, val,
	...
	const struct iio_chan_spec *chan)
{
	...
	/* Read Trig SRC */
	trig_src = val = m2k_la_read(st, M2K_LA_REG_INSTRUMENT_TRIGGER);
	trig_src = FIELD_GET(M2K_LA_TRIGGER_EXT_SOURCE_MASK, trig_src);
	...
	const struct iio_chan_spec *chan, unsigned int val)
{
	...
	m2k_la_update(st, M2K_LA_REG_INSTRUMENT_TRIGGER, 0x0,
	...
	const struct iio_chan_spec *chan)
{
	...
	val = m2k_la_read(st, M2K_LA_REG_INSTRUMENT_TRIGGER);
	...
	const struct iio_chan_spec *chan, unsigned int val)
{
	...
	mutex_lock(&st->lock);
	if (val)
		st->io_sel &= ~BIT(chan->address);
	else
		st->io_sel |= BIT(chan->address);
	m2k_la_write(st, M2K_LA_REG_IO_SEL, st->io_sel);
	mutex_unlock(&st->lock);
	...
	const struct iio_chan_spec *chan, unsigned int val)
{
	...
	mutex_lock(&st->lock);
	st->output_mode &= ~BIT(chan->address);
	st->output_mode |= val << chan->address;
	m2k_la_write(st, M2K_LA_REG_OUTPUT_MODE, st->output_mode);
	mutex_unlock(&st->lock);
	...
	const struct iio_chan_spec *chan)
{
	...
	return (st->output_mode >> chan->address) & 1;
}
	...
	const struct iio_chan_spec *chan, unsigned int val)
{
	...
	mutex_lock(&st->lock);
	m2k_la_write(st, M2K_LA_REG_CLOCK_SEL, val);
	st->clocksource = val;
	mutex_unlock(&st->lock);
	...
	unsigned int writeval, unsigned int *readval)
{
	...
	if (readval == NULL)
		m2k_la_write(st, reg, writeval);
	else
		*readval = m2k_la_read(st, reg);
...
{
	struct m2k_la_state *st = iio_device_get_drvdata(indio_dev);
	//FIXME uint32_t mask = *indio_dev->buffer->channel_mask;
	uint32_t mask = *indio_dev->buffer->scan_mask;

	m2k_la_write(st, M2K_LA_REG_GPO_EN, ~mask);
	return 0;
}
...
static int m2k_la_tx_postdisable(struct iio_dev *indio_dev)
{
	...
	m2k_la_write(st, M2K_LA_REG_GPO_EN, 0xffff);
	return 0;
}
...
static int m2k_la_probe(struct platform_device *pdev)
{
	...
	ret = devm_add_action_or_reset(&pdev->dev, m2k_la_disable_clk, st);
	if (ret)
		return ret;
	...
	m2k_la_write(st, M2K_LA_REG_IO_SEL, st->io_sel);
	m2k_la_write(st, M2K_LA_REG_GPO_EN, 0xffff);
	m2k_la_write(st, M2K_LA_REG_OUTPUT_MODE, 0x0);

	m2k_la_write(st, M2K_LA_REG_CLOCK_SEL, 0x0);
	m2k_la_write(st, M2K_LA_REG_FIFO_DEPTH, 0x0);
	m2k_la_write(st, M2K_LA_REG_TRIGGER_DELAY, 0x0);
	...
	/* FIXME: direction out */
	buffer_tx = devm_iio_dmaengine_buffer_alloc(&pdev->dev, "tx");
	if (IS_ERR(buffer_tx))
	...
	buffer_rx = devm_iio_dmaengine_buffer_alloc(&pdev->dev, "rx");
	if (IS_ERR(buffer_rx))
